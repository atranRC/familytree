//check session
//check sessionWikiRelation
//pass sessionWikiRelation as page prop
...
export default function MyWikidraftsPage({ sessionUserJson }) {
    ...
    const rows =
        data &&
        data.data.data.wikidrafts.map((r) => {
            let color = "yellow";
            let status = "In draft";
            if (r.wikiId) {
                color = "green";
                ...
                <tr key={r._id.toString()}>
                    ...
                    <td>
                        ...
                        <span
                            ...
                            onClick={() => {
                                router.push(
                                    `/profiles/${sessionUserJson._id}/my-wikis/drafts/${r._id}`
                                );
                            }}
                        >
                            edit
...
export async function getServerSideProps(context) {
    ...
    //fetch session user
    console.log("wiki-drafts", context.query.id);
    await dbConnect();
