...
export default function FeaturedStoriesFromPeopleGrid() {
    const { classes } = useStyles();
    const [selectedStory, setSelectedStory] = useState(null);
    const featuredStoriesQuery = useQuery({
        queryKey: ["fetch-featured-stories"],
        queryFn: () => {
            return axios.get(`/api/featured-user-stories`);
        },
    ...
    return (
        <div className={classes.treesSection}>
            ...
            {featuredStoriesQuery.data?.data.map((story, i) => {
                return (
                    <Paper
                        withBorder
                        p={10}
                    ...
                    >
                        <Stack spacing={3}>
                            ...
                            >
                                <Link
                                    href={`/timeline?articleId=${story.articleId}`}
                                >
                                    Go to article
                                </Link>
                            </Text>
